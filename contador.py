...
def cria_grafico(histograma):
    ...
    legendas = [chave for chave in sorted(maiores,
                                          key=maiores.get,
                                          reverse=True)]
    valores = [maiores[chave] for chave in legendas]

    explode = [0.1 if (item in ['java', 'py', 'js', 'nenhuma', 'outros'])
               else 0 for item in legendas]
...
if __name__ == "__main__":
    histograma = {}
    varre_diretorio(input('Caminho: '), histograma)
